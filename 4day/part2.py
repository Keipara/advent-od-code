...
def part1():
    ...
    for i in range(len(chosen_numbers)):
        current_number = chosen_numbers[i]
        ...
        for j in range(len(boards)):
            ...
            for k in range(len(board)):
                ...
                for l in range(len(row)):
                    if row[l] == current_number:

                        if f'board{j}row{k}' not in rows_dict:
                            rows_dict[f'board{j}row{k}'] = 1
                            row[l] = -1
                        else:
                            rows_dict[f'board{j}row{k}'] = rows_dict[f'board{j}row{k}'] + 1
                            row[l] = -1

                        if  rows_dict[f'board{j}row{k}'] == 5 and board in boards_copy and len(boards_copy) == 1:
                                loser = boards_copy[0][0] + boards_copy[0][1] + boards_copy[0][2] + boards_copy[0][3] + boards_copy[0][4]
                                print(loser)
                                for m in range(len(loser)):
                                    if loser[m] == -1:
                                        loser[m] = 0
                                loser_sum = sum(loser)
                                print('Row Loser:', loser_sum, current_number, loser_sum * current_number)
                                return current_number
                        if  rows_dict[f'board{j}row{k}'] == 5 and board in boards_copy:
                            boards_copy.remove(board)

                        if f'board{j}column{l}' not in columns_dict:
                            columns_dict[f'board{j}column{l}'] = 1
                            row[l] = -1
                        else:
                            columns_dict[f'board{j}column{l}'] = columns_dict[f'board{j}column{l}'] + 1
                            row[l] = -1

                        if columns_dict[f'board{j}column{l}'] == 5 and board in boards_copy and len(boards_copy) == 1:
                                loser = boards_copy[0][0] + boards_copy[0][1] + boards_copy[0][2] + boards_copy[0][3] + boards_copy[0][4]
                                for m in range(len(loser)):
                                    if loser[m] == -1:
                                        loser[m] = 0
                                loser_sum = sum(loser)
                                print('Column Loser:', loser_sum, current_number, loser_sum * current_number)
                                return current_number
                        if columns_dict[f'board{j}column{l}'] == 5 and board in boards_copy:
                            boards_copy.remove(board)
